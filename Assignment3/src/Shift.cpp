...
bool operator< (const Shift& lhs, const Shift& rhs) {
    if (lhs.day != rhs.day) return static_cast<unsigned>(lhs.day) < static_cast<unsigned>(rhs.day);
...
int hashCode(const Shift& shift) {
    return hashCode(static_cast<unsigned>(shift.day), shift.startHour, shift.endHour);
}
