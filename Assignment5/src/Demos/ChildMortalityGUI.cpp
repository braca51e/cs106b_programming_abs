...
namespace {
    const string kDataFile = "UN-ChildMortality.csv";
...
struct ChildMortalityGUI::Impl {
    ...
    /* Reference back to the owning object. */
    ChildMortalityGUI* const mOwner;
    ...
    /* Constructs an implementation for a given window. */
    Impl(ChildMortalityGUI* owner, GWindow& window);
...
ChildMortalityGUI::Impl::Impl(ChildMortalityGUI* owner, GWindow& window) :
...
void ChildMortalityGUI::Impl::actionPerformed(GObservable* source) {
    if (source == mLowest) {
...
void ChildMortalityGUI::Impl::chooseBestCountriesBy(Scorer scorer, Formatter formatter) {
...
void ChildMortalityGUI::Impl::repaint(GWindow& window) {
...
void ChildMortalityGUI::Impl::drawGraph(GWindow& window, const GRectangle& bounds) const {
...
void ChildMortalityGUI::Impl::drawPlot(GWindow& window, const GRectangle& bounds) const {
...
void ChildMortalityGUI::Impl::drawLegend(GWindow& window, const GRectangle& bounds) const {
    ...
    LegendRender::construct(labels, kPlottedLineColors, {
                                bounds.getX() + bounds.getWidth() - kLegendWidth, bounds.getY(),
                                kLegendWidth, kLegendHeight
                            }, kLegendFontColor, kLegendFont, kLegendBorderColor)->draw(window);
}

/* The actual ChildMortalityGUI just forwards things to the implementation. */
ChildMortalityGUI::ChildMortalityGUI(GWindow& window) {
    mImpl = make_shared<Impl>(this, window);
}
