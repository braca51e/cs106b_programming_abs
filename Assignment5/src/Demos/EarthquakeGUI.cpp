...
namespace {
    ...
    const string kWorldFile = "mollweide-90.png";
    ...
    const double kDescriptionHeight = 100;
    ...
    /* Legend parameters. */
    const double kLegendPadding     = 15;
    ...
    Vector<Earthquake> largestEarthquakesIn(const Vector<Earthquake>& quakes, int k) {
        stringstream stream;
        for (size_t i = 0; i < quakes.size(); i++) {
...
struct EarthquakeGUI::Impl {
    EarthquakeGUI* const mOwner;
    ...
    Impl(GWindow& window, EarthquakeGUI* owner);
    void actionPerformed(GObservable* source);
    void repaint(GWindow& window);
};

EarthquakeGUI::Impl::Impl(GWindow& window, EarthquakeGUI* owner) :
    mOwner(owner),
...
void EarthquakeGUI::Impl::actionPerformed(GObservable* source) {
    if (source == mHour) {
...
void EarthquakeGUI::Impl::repaint(GWindow& window) {
...
void EarthquakeGUI::Impl::showEarthquakesFrom(const string& url) {
