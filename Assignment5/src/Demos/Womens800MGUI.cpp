...
namespace {
    const string kBaseDirectory = "800m/";
...
struct Womens800MGUI::Impl {
    /* Pointer back to the owner. */
    Womens800MGUI* const mOwner;
    ...
    Impl(GWindow& window, Womens800MGUI* owner);
    void changeOccurredIn(GObservable* source);
    void repaint(GWindow& window);
};

Womens800MGUI::Impl::Impl(GWindow& window, Womens800MGUI* owner)
    : mOwner(owner) {
...
void Womens800MGUI::Impl::changeOccurredIn(GObservable* source) {
    if (source == mYearSlider) {
        showBestTimesThrough(mYearSlider->getValue());
    }
}

void Womens800MGUI::Impl::showBestTimesThrough(int year) {
     ...
     * stop point.
     */
    int endPoint = lowerBoundOf(mYearPoints, year + 1);
...
void Womens800MGUI::Impl::repaint(GWindow& window) {
    /* Clear the display. */
